...
Render the app. On the server, you **must** use the `createMemoryHistory` and set the `initialEntries:` value to the request url. Invoke `dispatchOnServer()` to asynchronously load data before rendering:
...
Use the `withDispatcher` method to assign a _route dispatcher_ to a RouteComponent. Dispatch the route action(s) that will load all data required for rendering the route:
```js
// HomePageRouteComponent.js
import { withDispatcher } from 'react-router-dispatcher';
import HomePageContainer from './HomePageContainer';
...
When rendering for the browser, use the `createBrowserHistory`:
...
Render the application on the server using `createMemoryHistory` and invoke `dispatchOnServer` to asynchrously populate the redux store before rendering.
...
When configuring the client for HMR, you must update both the root application component **and the route configuration**.

```js
// client.js
import { AppContainer } from 'react-hot-loader'
// import store, routes, history...

function renderApp(ApplicationComponent, store, routes) {
  render(
    <AppContainer>
      <ApplicationComponent store={store} routes={routes} history={history} />
    </AppContainer>
...
function loadApp(store) {
  ...
  // initial render
  renderApp(App, store, routes);

  // The following is needed so that we can support hot reloading our application.
  if (process.env.NODE_ENV === 'development' && module.hot) {

    function reloadApp() {
      // Ensure both the App and routes are updated
      renderApp(require('../shared/app/App').default, store, require('../shared/routes').default);
    }

    // Changes to this file trigger reloading.
    module.hot.accept('./client.js');

    // Any changes to the App or routes will cause a re-render.
    module.hot.accept('../shared/routes', () => reloadApp());
    module.hot.accept('../shared/app/App', () => reloadApp());
  }
}
